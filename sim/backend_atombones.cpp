    ...
    simstate_(&(*(sim->simstate_)))
{
    ...
    try
    {
        mem_["imem"] = std::shared_ptr<Memory> (new Memory(config_.imem_size_kb * 1024, 0x00000000, true));
        mem_["dmem"] = std::shared_ptr<Memory> (new Memory(config_.dmem_size_kb * 1024, 0x04000000, false));
...
{    
    ...
    {
        bool success = false;
        for (auto element: mem_)
        {
            Memory * m = element.second;
...
int Backend_atomsim::tick()
{
    ...
    if (tb->m_core->AtomBones->atom_core->InstructionRegister == OPCODE_EBREAK)
    {
